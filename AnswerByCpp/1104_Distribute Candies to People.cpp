...
给第一个小朋友 1 颗糖果，第二个小朋友 2 颗，依此类推，直到给最后一个小朋友 n 颗糖果。 然后，我们再回到队伍的起点，
...
解释： 第一次，ans[0] += 1，数组变为 [1,0,0]。   第二次，ans[1] += 2，数组变为 [1,2,0]。
第三次，ans[2] += 3，数组变为 [1,2,3]。   第四次，ans[0] += 4，最终数组变为 [5,2,3]。
...
public:
    vector<int> distributeCandies(int candies, int num_people) {
        ...
        int cur = 0, num = 1;
        while(candies != 0){
            if(candies > num) res[cur] += num, candies -= num++;
            else res[cur] += candies, candies = 0;
            cur = (cur+1) % num_people;
        }
        return res;
    }
};
...
int main(){
    vector<int> arr = Solution().distributeCandies(10, 3);
