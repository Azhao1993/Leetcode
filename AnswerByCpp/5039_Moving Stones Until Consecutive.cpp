...
从位置 x 或者是位置 z 拿起一枚石子，并将该石子移动到某一整数位置 k 处，
其中 x < k < z 且 k != y。
...
public:
    vector<int> numMovesStones(int a, int b, int c) {
        ...
        int min_moves = min(1,maxN-midN-1) + min(1, midN-minN-1);
        int max_moves = maxN-midN-1 + midN-minN-1;
        
        vector<int> arr;
        arr.push_back(min_moves);
        arr.push_back(max_moves);
        return arr;
    }
};
