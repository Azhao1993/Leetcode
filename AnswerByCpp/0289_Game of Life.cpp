...
进阶: 你可以使用原地算法解决本题吗？请注意，面板上所有格子需要同时被更新：你不能先更新某些格子，
然后使用它们的更新后的值再更新其他格子。 本题中，我们使用二维数组来表示面板。原则上，面板是无限的，
    ...
    void gameOfLife(vector<vector<int>>& board) {
        for(int i=0; i<board.size(); i++)
            for(int j=0; j<board[0].size(); j++){
                ...
                if(board[i][j] == 1 && (num < 2 || num > 3)) board[i][j] = 3;
            }
        ...
        for(int i=x-1; i<=x+1; i++)
            for(int j=y-1; j<=y+1; j++){
                if(i<0 || j<0 || i>=board.size() || j>=board[0].size())
                    continue;
                if(board[i][j]%2 == 0 || (x == i && y == j)) continue;
                num++;
            }
        return num;
    }
