...
第二个序列是因为它的最后一个差值为零。给定一个整数序列，返回作为摆动序列的最长子序列的长度。 
...
public:
    int wiggleMaxLength(vector<int>& nums) {
        ...
        // 防止有相等的元素
        while(cur < len && nums[cur] == sta.top()) cur++;
        if(cur == len) return 1;

        flag = nums[cur] > sta.top() ? true : false;
        sta.push(nums[cur++]);
        for(; cur < len; cur++){
            if(nums[cur] == sta.top()) continue;
            // 发现连续元素，弹出之前的，把现在的进栈
            if((nums[cur] > sta.top() && flag) || (nums[cur] < sta.top() && !flag))
                sta.pop();
            // 若非连续元素，改变指向，把现在的进栈
            else flag = !flag;
            // 无论什么情况都会指向
...
int main(){
    ...
    int num = Solution().wiggleMaxLength(arr);
    cout<<num<<endl;
    return 0;
}