...
示例 3：   输入：A = "leetcode", B = "programs", S = "sourcecode"     输出："aauaaaaada"
解释：我们可以把 A 和 B 中的等价字符分为 [a,o,e,r,s,c], [l,p], [g,t] 和 [d,m] 共 4 组，
因此 S 中除了 'u' 和 'd' 之外的所有字母都转化成了 'a'，最后答案为 "aauaaaaada"。

提示： 字符串 A，B 和 S 仅有从 'a' 到 'z' 的小写英文字母组成。
字符串 A，B 和 S 的长度在 1 到 1000 之间。   字符串 A 和 B 长度相同。
*/

class Solution {
private:
    unordered_map<char, char> fatherMap;
    char findFather(char ch){
        char fa = fatherMap[ch];
        if(fa != ch) fa = findFather(fa);
        fatherMap[ch] = fa;
        return fa;
    }
    void unionFather(char a, char b){
        char aFa = findFather(a);
        char bFa = findFather(b);
        fatherMap[aFa] = min(aFa, bFa);
        fatherMap[bFa] = min(aFa, bFa);
    }
public:
...
int main(){
    ...
    string res = Solution().smallestEquivalentString(A,B,S);
    cout<<res<<endl;
