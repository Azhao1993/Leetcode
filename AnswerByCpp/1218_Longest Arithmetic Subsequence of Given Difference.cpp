...
给你一个整数数组 arr 和一个整数 difference，请你找出 arr 中所有相邻元素之间的差等于给定 
...
提示：     1 <= arr.length <= 10^5         -10^4 <= arr[i], difference <= 10^4
*/
...
public:
    int longestSubsequence(vector<int>& arr, int diff) {
        ...
        for(auto it:arr)
            res = max(res, has[it] = max(has[it], has[it - diff] + 1));
        return res;
    }
};
...
int main(){
    ...
    int res = Solution().longestSubsequence(arr, -2);
    cout << res << endl;
    return 0;
}