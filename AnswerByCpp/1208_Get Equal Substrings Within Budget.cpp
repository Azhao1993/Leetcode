...
也就是两个字符的 ASCII 码值的差的绝对值。 用于变更字符串的最大预算是 maxCost。在转化字符串时，总开销应当小于等于该预算，
这也意味着字符串的转化可能是不完全的。 如果你可以将 s 的子字符串转化为它在 t 中对应的子字符串，则返回可以转化的最大长度。
...
public:
    int equalSubstring(string s, string t, int maxCost) {
        int arr[100005], len = s.size(), res = 0;
        for(int i=0; i < len; i++)
            arr[i+1] = arr[i] + max(s[i]-t[i], t[i]-s[i]);
        for(int i=1,j=0; i<=len; i++) {
            while(arr[i] - arr[j] > maxCost) j++;
            res = max(res, i - j);
        }
        return res;
    }
