...
public:
    vector<int> numMovesStones(int a, int b, int c) {
        ...
        int min_moves = 2, max_moves = arr[2]-arr[0]-2;
        // 不需移动
        if(arr[2] == arr[0]+2) min_moves = 0;
        // 能够一步到位的情况
        else if(arr[1] <= arr[0]+2 || arr[2] <= arr[1]+2) min_moves = 1;
...
int main(){
    ...
    vector<int> arr = so->numMovesStones(1,4,67);
