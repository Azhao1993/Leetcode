...
public:
    ...
    int maxProfit(vector<int>& prices) {
        int len = prices.size();
        if(len==0)return 0;
        // 存放当前从最左处和最右处买到中间的比较值
        vector<int> leftBuy(len, 0), rightBuy(len, 0);
        int minValue = prices[0], maxValue = prices[len-1];
        int res = 0;
        for(int i=1; i<len; i++){
            minValue = min(minValue, prices[i]);
            leftBuy[i] = max(leftBuy[i-1],prices[i] - minValue);
        }
        for(int i=len-2; i>=0; i--){
            maxValue = max(maxValue, prices[i]);
            rightBuy[i] = max(rightBuy[i+1], maxValue - prices[i]);
        }
        for(int i=0; i<len; i++)
            // 若当前值为都在 i  进行了操作， 即为只购买了一次
            res = max(res, leftBuy[i]+rightBuy[i]);
    ...
    int maxProfit(vector<int>& prices) {
        ...
        for(int i=prices.size()-1; i>=0; i--)
            for(int j=2; j>=0; j--)
                for(int k=1; k>=0; k--){
                    if(j==2 && k==0) continue ;
                    if(k > 0) dp[i][j][1] = max(dp[i+1][j][1], dp[i+1][j][0]+prices[i]);
    ...
    int maxProfit(vector<int>& prices) {
        int res = 0;
        // 0   下标     0   买了几次    false    持有状态
        return dfs(prices, 0, 0, false);
    }
    int dfs(vector<int>& prices, int i, int times, bool flag){
        if(i==prices.size() || (times==2 && !flag)) return 0;

        if(flag)
            // 选择卖或不卖
            return max(dfs(prices, i+1, times, false)+prices[i], dfs(prices, i+1, times, flag));
        else
            // 选择买或不买，买入操作的时候，times才会 + 1
            return max(dfs(prices, i+1, times+1, true)-prices[i], dfs(prices, i+1, times, flag));
    }
    /*
    // 我实现的暴力，太暴力了，通过198/200 测试用例
    int maxProfit(vector<int>& prices) {
        int res = 0;
        // 0   下标     0   买了几次    0    tem
        dfs(res, prices, 0, 0, 0);
        return res;
    }
    void dfs(int &res, vector<int>& prices, int i, int times, int tem){
        // 每进行一次卖出操作就判断一下
        if(times%2==0) res = max(res, tem);
        if(times==4)return ;
        if(i==prices.size()) return ;
        // 买入操作
        if(times%2==0) dfs(res, prices, i+1, times+1, tem-prices[i]);
        // 卖出操作
        else dfs(res, prices, i+1, times+1, tem+prices[i]);
        // 什么都不做
        dfs(res, prices, i+1, times, tem);
    }
    */
};
