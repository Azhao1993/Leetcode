...
List<String> input(char c): 其中 c 是用户输入的下一个字符。字符只会是小写英文字母（'a' 到 'z' ），
空格（' '）和特殊字符（'#'）。输出历史热度前三的具有相同前缀的句子。

样例 ： 操作 ： AutocompleteSystem(["i love you", "island","ironman", "i love leetcode"], [5,3,2,2])
系统记录下所有的句子和出现的次数：
"i love you" : 5 次
"island" : 3 次
"ironman" : 2 次
...
解释 ：有四个句子含有前缀 "i"。其中 "ironman" 和 "i love leetcode" 有相同的热度，由于 ' ' 的 ASCII 码是 32 而 'r' 的 ASCII 码是 114，
...
注释 ： 输入的句子以字母开头，以 '#' 结尾，两个字母之间最多只会出现一个空格。
...
public:
    ...
    void insert (string& input, int time) {
        Node* cur = root;
        for (auto &ch : input) {
            if (cur->words.find(ch) == cur->words.end())
                cur->words[ch] = new Node();
            cur = cur->words[ch];
        }
        cur->count += time;
        cur->str = input;
    }
    void search(Node* cur, vector<pair<int, string>> &res) {
        if (cur == nullptr) return ;
        if (cur->count > 0) res.push_back({-cur->count, cur->str});
        for (auto &p : cur->words) search(p.second, res);
    }
};
...
public:
    ...
    vector<string> input(char c) {
        ...
        if (c == '#') {
            ...
            cur += c;
            if (root->words.find(c) == root->words.end())
                root->words[c] = new Node();
            root = root->words[c];
            vector<pair<int, string>> resu;
            trie->search(root, resu);
            sort(resu.begin(), resu.end());
            for (int i=0; i<min((int)resu.size(), 3); i++)
                res.push_back(resu[i].second);
            return res;
        }
    }
};
