...
段式回文 其实与 一般回文 类似，只不过是最小的单位是 一段字符 而不是 单个字母。 举个例子，对于一般回文 "abcba" 是回文，
而 "volvo" 不是，但如果我们把 "volvo" 分为 "vo"、"l"、"vo" 三段，则可以认为 “(vo)(l)(vo)” 是段式回文（分为 3 段）。
给你一个字符串 text，在确保它满足段式回文的前提下，请你返回 段 的 最大数量 k。 如果段的最大数量为 k，
...
提示：     text 仅由小写英文字符组成。        1 <= text.length <= 1000
*/
...
public:
    ...
    int helper(string text, int left, int right){
        ...
        while(left+length <= right-length+1){
            if(text.substr(left, length) == text.substr(right-length+1, length))
...
int main(){
    int res = Solution().longestDecomposition("dsaf");
    cout<<res<<endl;
    return 0;
}